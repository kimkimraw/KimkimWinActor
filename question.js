...
let questions = [
	{
		...
		[
			...
			},
			{
				...
				"q_Select":[
					["Windows上で動作するアプリケーションを人間に代わり操作するソフトウェア型ロボット",0,1],
					["事前にフローチャートを作成することで自動でシナリオを作成することができる",1,2],
					["IEモードでの記録は、IEのバージョン制限がある",0,3],
					["記録、編集、実行の機能がある",0,4],
				],
			...
			},
			{
				...
				"q_Select":[
					["Windows上で動作するアプリケーションを人間に代わり操作するソフトウェア型ロボット",0,1],
					["事前にフローチャートを作成することで自動でシナリオを作成することができない",0,2],
					["IEモードでの記録は、IEのバージョン制限はない",1,3],
					["記録、編集、実行の機能がある",0,4],
				],
			...
			},
			{
				...
				"q_Select":[
					["WinActor.exeを再起動",0,1],
					["コンピュータ→マイドキュメントの中から探す",0,2],
					["デスクトップの中から探す",0,3],
					["タスクトレイのWinActorのアイコンをクリックする",1,4],
				],
			...
			},
			{
				...
				"q_Select":[
					["IEのバージョンを合わせる必要がある",0,1],
					["ExcelなどのOffice製品のバージョンを合わせる必要がある",0,2],
					["「画像マッチング」など画像を使用するシナリオを作成する際は、OSごとの解像度を意識する必要がある",1,3],
					["画面幅による画像マッチングの動作不正をしないようにシナリオを作成する必要がある",0,4],
				],
			...
			},
			{
				...
				"q_Select":[
					["「UI識別型」「イベントモード」「Chromeモード」「座標指定型」",0,1],
					["「UI識別型」「ファイル向け」「画像識別型」「エミュレーションモード」",0,2],
					["「画像識別型」「UI識別型」「ファイル向け」「座標指定型」",1,3],
					["「画像識別型」「UI識別型」「ファイル向け」「エミュレーションモード」",0,4],
				],
