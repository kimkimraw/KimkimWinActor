...
let questions = [
	{
		...
		[
			...
			{
				"q_Titile":"WinActorの説明として正しいものを選びなさい。",
				"q_Select":[
					["Windows上で動作するアプリケーションを人間に代わり操作するソフトウェア型ロボット",0],
					["事前にフローチャートを作成することで自動でシナリオを作成することができない",0],
					["IEモードでの記録は、IEのバージョン制限がある",0],
			...
			},
			{
				...
				"q_Select":[
					...
					["「IEモード」「ファイル向け」「画像識別型」「エミュレーションモード」",0],
		...
		[
			...
			},
			{
				...
				"q_Select":[
					...
					["ファンクションキー操作の追加",0],
					["待機時間の追加",0],
					["英字の大文字・小文字を区別した登録が可能",1],
				],
				"q_Desctiption":"エミュレーションモードで自動記録後に作成したエミュレーションのプロパティ編集機能にはマウス操作・ファンクションキー操作・待機時間の追加が可能ですが、英字の大文字・小文字を区別して登録することはできません。"
			},
			{
				"q_Titile":"WinActorの画面構成として正しいものを選びなさい。",
				"q_Select":[
					["フローチャート画面、監視ルール画面、イメージ画面、変数一覧画面、データ一覧画面、ログ出力画面",0],
					["メイン画面、フローチャート画面、監視ルール画面、イメージ画面、変数一覧画面、データ一覧画面、ログ出力画面",1],
					["メイン画面、フローチャート画面、監視ルール画面、イメージ画面、変数一覧画面、データ出力画面、ログ出力画面",0],
					["メイン画面、監視ルール画面、イメージ画面、変数一覧画面、データ一覧画面、ログ出力画面",0],
				],
				"q_Desctiption":"画面構成はメイン画面、フローチャート画面、監視ルール画面、イメージ画面、変数一覧画面、データ一覧画面、ログ出力画面です。"
			},
			{
				"q_Titile":"メイン画面のオプションに含まれていない機能を選択しなさい。",
				"q_Select":[
					["予期せぬマウス／キーボード操作による一時停止する。",0],
					["常にログを出力する。",0],
					["変数を自動生成する。",0],
					["スクリーンセーバーによる画面ロックがされた場合に自動的にロック解除する",1],
				],
				"q_Desctiption":"メイン画面のオプションにはスクリーンセーバによる画面ロックされた場合、自動解除するようなものは存在しません。存在するのは、予期せぬマウス・キーボード操作による一時停止、常にログを出力する、変数を自動生成する、などがあります。"
			},	
		],
	},
	{
		...
		[
			...
			{
				"q_Titile":"ノード【繰り返し】を使用する場合設定できないものはどれか？",
				"q_Select":[
					["条件式",0],
					["データ数",0],
					["繰り返し回数",0],
				...
				"q_Desctiption":" ノード【繰り返し】で設定可能なプロパティは次の5つです。条件式、繰り返し回数、範囲、データ数、データ数（DB連携）"
			},
		],
	},
];
